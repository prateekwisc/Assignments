...
__shared__ float sN[BLOCK_SIZE + radius * 2][BLOCK_SIZE + radius * 2];
...
if(i<0 || j<0)
  ...
  sN[ty][tx] = N.elements[tid - radius - (N.width * radius)];
...
sN[ty][tx + 2*radius] = N.elements[tid + radius - (N.width * radius)];
...
sN[ty + 2* radius][tx] = N.elements[tid - radius + (N.width * radius)];

i = x + radius;
j = y + radius;
if ( i > N.width-1 || j > N.height-1)
sN[ty + 2*radius][tx + 2*radius] = 0;
else 
sN[ty + 2*radius][tx + 2*radius] = N.elements[tid + radius + (N.width * radius)]; 
